...
SEXP subsetFSTBare(const std::string& rootPath, const List& subparsed, const NumericVector& dim, const SEXPTYPE& dtype) {
  ...
  switch(dtype){
  case REALSXP: 
    res = subsetFST_double(rootPath_alt, dim, subparsed);
    break;
  case INTSXP:
    res = subsetFST_integer(rootPath_alt, dim, subparsed);
    break;
  case STRSXP:
  case CHARSXP:
    res = subsetFST_character(rootPath_alt, dim, subparsed);
    break;
  case CPLXSXP:
    res = subsetFST_complex(rootPath_alt, dim, subparsed);
    break;
  default:
