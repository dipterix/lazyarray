...
public:
  ...
  virtual bool validate(bool stopIfError = true) {
    bool isValid = true;
    isValid = isValid && stopIfNot(dimension.size() >= 2, "LazyArray must have dimension size >= 2", stopIfError);
    isValid = isValid && stopIfNot(*(dimension.end() - 1) == _nparts, "LazyArray dimensions inconsistent with number of partitions", stopIfError);
...
public:
  ...
  bool validate(bool stopIfError = true) override {
    ...
    bool isValid = true;
    isValid = isValid && stopIfNot(dimension.size() >= 2, "FstArray must dimension >= 2", stopIfError);
    isValid = isValid && stopIfNot(*(dimension.end() - 1) == _nparts, "FstArray dimensions inconsistent with number of partitions", stopIfError);
