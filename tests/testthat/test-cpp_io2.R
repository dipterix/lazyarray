...
loader_f <- function(..., samp, reshape, drop){
  lazyarray:::lazySubset(a$get_partition_fpath(), environment(), dim, getSexpType(samp), reshape, drop)
}
...
test_that("Loader2 with matrix", {
  ...
  a <- as.lazyarray(x)
  loader_f <- function(i, ...){
    lazyarray:::lazySubset(a$get_partition_fpath(), environment(), dim(x), getSexpType(1L), NULL, FALSE)
  }
